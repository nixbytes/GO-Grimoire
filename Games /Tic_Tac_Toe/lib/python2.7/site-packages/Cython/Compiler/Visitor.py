    ...
    * builtin methods: _handle_(general|simple|any)_method_TYPENAME_METHODNAME
    ...
    def _dispatch_to_handler(self, node, function, arg_list, kwargs):
        if function.is_name:
            ...
            if not function.entry:
                return node
            is_builtin = function.entry.is_builtin or function.entry is self.current_env().builtin_scope().lookup_here(
                function.name
            )
            if not is_builtin:
    ...
    def _dispatch_to_method_handler(
        ...
        if method_handler is None:
            if attr_name in TypeSlots.method_name_to_slot or attr_name == "__new__":
