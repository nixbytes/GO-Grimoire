...
class InterpretCompilerDirectives(CythonTransform, SkipDeclarations):
        ...
        if node.module_name == u"cython":
            ...
            if node.module_name == u"cython.parallel":
                if node.as_name and node.as_name != u"cython":
                    ...
                    self.parallel_directives[u"cython.parallel"] = node.module_name
                self.module_scope.use_utility_code(
                    UtilityCode.load_cached("InitThreads", "ModuleSetupCode.c")
                )
            elif node.as_name:
            ...
            for pos, name, as_name, kind in node.imported_names:
                ...
                if self.is_parallel_directive(qualified_name, node.pos):
                    ...
                    if kind is not None:
                        self.context.nonfatal_error(
                            PostParseError(
                                pos, "Compiler directive imports must be plain imports"
                            )
                        )
                else:
    ...
    def visit_FromImportStatNode(self, node):
        if (
            node.module.module_name.value == u"cython"
        ) or node.module.module_name.value.startswith(u"cython."):
    ...
    def visit_SingleAssignmentNode(self, node):
        if isinstance(node.rhs, ExprNodes.ImportNode):
            ...
            node = Nodes.CImportStatNode(
                node.pos, module_name=module_name, as_name=as_name
            )
    ...
    def visit_with_directives(self, body, directives):
        ...
        directive = Nodes.CompilerDirectivesNode(
            pos=retbody.pos, body=retbody, directives=newdirectives
        )
    ...
    def _extract_directives(self, node, scope_name):
        ...
        for dec in node.decorators:
            ...
            if new_directives is not None:
                ...
                realdecs.append(dec)
        if realdecs and isinstance(
            node, (Nodes.CFuncDefNode, Nodes.CClassDefNode, Nodes.CVarDefNode)
        ):
            raise PostParseError(
                realdecs[0].pos,
                    ...
                    Nodes.IfClauseNode(
                        ...
                        condition=ExprNodes.NotNode(
                            ...
                            operand=ExprNodes.WithExitCallNode(
                                ...
                                test_if_run=False,
                                args=excinfo_target,
                                await=ExprNodes.AwaitExprNode(pos, arg=None)
                                if is_async
                                else None,
                            ),
                        ),
        ...
        for decorator_node in node.decorators[::-1]:
            ...
            if decorator.is_name and decorator.name == "property":
                ...
                if handler_name:
                    ...
                    if len(node.decorators) > 1:
                        return self._reject_decorated_property(node, decorator_node)
                    return self._add_to_property(
                        properties, node, handler_name, decorator_node
                    )
...
if VALUE is not None:
    ...
    def _inject_pickle_methods(self, node):
        ...
        if cinit or non_py or (structs and not auto_pickle_forced):
            ...
            checksum = (
                "0x%s"
                % hashlib.md5(" ".join(all_members_names).encode("utf-8")).hexdigest()[
                    :7
                ]
            )
    ...
    def visit_CNameDeclaratorNode(self, node):
        if node.name in self.seen_vars_stack[-1]:
            ...
            if (
                ...
                warning(
                    node.pos,
                    "cdef variable '%s' declared after it is used" % node.name,
                    2,
                )
        self.visitchildren(node)
        return node
    ...
    def visit_DefNode(self, node):
        ...
        if "ccall" in self.directives:
            node = node.as_cfunction(
                overridable=True,
                returns=self.directives.get("returns"),
                modifiers=modifiers,
            )
...
class MarkClosureVisitor(CythonTransform):
    ...
    def visit_FuncDefNode(self, node):
        ...
        gbody = Nodes.GeneratorBodyDefNode(pos=node.pos, name=node.name, body=node.body)
        coroutine = (
            Nodes.AsyncDefNode if node.is_async_def else Nodes.GeneratorDefNode
        )(
            pos=node.pos,
            name=node.name,
            args=node.args,
