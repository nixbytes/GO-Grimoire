...
from .DebugFlags import debug_disposal_code, debug_temp_alloc, debug_coercion
from .Pythran import (
    to_pythran,
    ...
    def coerce_to_boolean(self, env):
        ...
        if type.is_enum or type.is_error:
            ...
            return SimpleCallNode(
                self.pos,
                function=AttributeNode(self.pos, obj=self, attribute="operator bool"),
                args=[],
            ).analyse_types(env)
        elif type.is_ctuple:
...
class NewExprNode(AtomicExprNode):
    ...
    def infer_type(self, env):
        ...
        if constructor is None:
            func_type = PyrexTypes.CFuncType(type, [], exception_check="+", nogil=True)
...
class NameNode(AtomicExprNode):
    ...
    def check_const(self):
        ...
        if entry is not None and not (
            entry.is_const or entry.is_cfunction or entry.is_builtin
        ):
            self.not_const()
            return False
        return True
    ...
    def generate_result_code(self, code):
        ...
        if entry.is_builtin and entry.is_const:
            ...
            code.putln(
                "%s = __Pyx_GetModuleGlobalName(%s);" % (self.result(), interned_cname)
            )
            if not self.cf_is_null:
                code.putln("}")
            ...
            if (
                null_code
                and raise_unbound
                and (entry.type.is_pyobject or memslice_check)
            ):
        ...
        if entry.is_pyglobal:
            ...
            if self.type.is_pyobject:
                ...
                if self.use_managed_ref:
                    ...
                    assigned = True
                    if entry.is_cglobal:
                        code.put_decref_set(self.result(), rhs.result_as(self.ctype()))
                    else:
                        if not self.cf_is_null:
                            if self.cf_maybe_null:
        ...
        if self.entry is None:
            ...
            if not self.cf_is_null:
                ...
                if self.entry.type.is_pyobject:
                    ...
                    code.put_xdecref_memoryviewslice(
                        self.entry.cname, have_gil=not self.nogil
                    )
        else:
...
class ImportNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        if (
            self.level <= 0
            and self.module_name.is_string_literal
            and self.module_name.value in utility_code_for_imports
        ):
            helper_func, code_name, code_file = utility_code_for_imports[
...
class IteratorNode(ExprNode):
    ...
    def analyse_cpp_types(self, env):
        ...
        if iter_type.is_cpp_class:
            if (
                ...
                error(
                    self.pos,
                    "missing operator!= on result of begin() on %s"
                    % self.sequence.type,
                )
                self.type = error_type
                return
            ...
            if env.lookup_operator_for_types(self.pos, "*", [iter_type]) is None:
                error(
                    self.pos,
                    "missing operator* on result of begin() on %s" % self.sequence.type,
                )
                self.type = error_type
                return
            self.type = iter_type
            ...
            error(
                self.pos,
                "result type of begin() on %s must be a C++ class or pointer"
                % self.sequence.type,
            )
            self.type = error_type
            return
    ...
    def generate_result_code(self, code):
        ...
        if sequence_type.is_cpp_class:
            if self.sequence.is_name:
                ...
                self.cpp_iterator_cname = code.funcstate.allocate_temp(
                    sequence_type, manage_ref=False
                )
                code.putln(
                    "%s = &%s;" % (self.cpp_iterator_cname, self.sequence.result())
                )
    ...
    def generate_iter_next_result_code(self, result_name, code):
        ...
        if sequence_type.is_cpp_class:
            ...
            # TODO: Cache end() call?
            code.putln("if (!(%s != %s())) break;" % (self.result(), end_func))
            code.putln("%s = *%s;" % (result_name, self.result()))
...
class WithExitCallNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        if self.await:
            ...
            self.await.generate_evaluation_code(
...
class MemoryCopyScalar(MemoryCopyNode):
    ...
    def _generate_assignment_code(self, scalar, code):
        ...
        slice_iter_obj = MemoryView.slice_iter(
            self.dst.type, dst_temp, self.dst.type.ndim, code
        )
    ...
    def generate_result_code(self, code):
        if self.is_literal:
            self.result_code = code.get_py_const(
                py_object_type, "slice", cleanup_level=2
            )
            code = code.get_cached_constants_writer()
            code.mark_pos(self.pos)
...
class SimpleCallNode(CallNode):
    ...
    def analyse_types(self, env):
        ...
        if self.is_numpy_call_with_exprs:
            ...
            self.type = PythranExpr(
                pythran_func_type(self.function.attribute, self.arg_tuple.args)
            )
            ...
            self.args = None
            if (
                func_type is Builtin.type_type
    ...
    def analyse_c_function_call(self, env):
        ...
        if self.function.is_name or self.function.is_attribute:
            ...
            if func_entry and (
                ...
                self.is_temp = (
                    1
                )  # currently doesn't work for self.calculate_result_code()
    ...
    def generate_result_code(self, code):
        ...
        if func_type.is_pyobject:
            if (
                func_type is not type_type
                and not self.arg_tuple.args
                and self.arg_tuple.is_literal
            ):
                code.globalstate.use_utility_code(
...
class PyMethodCallNode(SimpleCallNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        if self.is_numpy_call_with_exprs:
            ...
            code.putln(
                "new (&%s) decltype(%s){pythonic::numpy::functor::%s{}(%s)};"
                % (
                    self.result(),
                    self.result(),
...
class GeneralCallNode(CallNode):
    ...
    def map_to_simple_call_node(self):
        ...
        matched_args = set(
            [arg.name for arg in declared_args[: len(pos_args)] if arg.name]
        )
...
class AttributeNode(ExprNode):
            ...
            if type.is_extension_type or type.is_builtin_type or type.is_cpp_class:
                ...
                if entry and (
                    ...
                    if type.is_builtin_type:
                        ...
                        if entry.func_cname and not hasattr(
                            ...
                            if entry.type.is_static_method or (
                                ...
                                error(
                                    self.pos,
                                    "%s not a static member of %s" % (entry.name, type),
                                )
                                ...
                                ctype.args[0] = PyrexTypes.CFuncTypeArg(
                                    "self", type, "self", None
                                )
                        else:
    ...
    def analyse_as_type(self, env):
        ...
        if not self.obj.is_string_literal:
            ...
            if (
                base_type
                and hasattr(base_type, "scope")
                and base_type.scope is not None
            ):
    ...
    def generate_disposal_code(self, code):
        if self.is_temp and self.type.is_memoryviewslice and self.is_memslice_transpose:
            ...
            code.put_xdecref_memoryviewslice(self.result(), have_gil=True)
            code.putln("%s.memview = NULL;" % self.result())
            code.putln("%s.data = NULL;" % self.result())
        else:
    ...
    def generate_assignment_code(
        ...
        if self.is_py_attr:
            ...
            if not self.type.is_memoryviewslice:
                code.putln("%s = %s;" % (select_code, rhs.result_as(self.ctype())))
                # rhs.result()))
            rhs.generate_post_assignment_code(code)
            rhs.free_temps(code)
...
class TupleNode(SequenceNode):
    ...
    def infer_type(self, env):
        ...
        arg_types = [arg.infer_type(env) for arg in self.args]
        if any(
            type.is_pyobject or type.is_unspecified or type.is_fused
    ...
    def analyse_types(self, env, skip_children=False):
        ...
        if not self.mult_factor and not any(
            (arg.is_starred or arg.type.is_pyobject or arg.type.is_fused)
            for arg in self.args
        ):
    ...
    def coerce_to(self, dst_type, env):
        if self.type.is_ctuple:
            if dst_type.is_ctuple and self.type.size == dst_type.size:
                ...
                return TupleNode(
                    self.pos, args=coerced_args, type=tuple_type, is_temp=1
    ...
    def generate_operation_code(self, code):
        ...
        if self.is_partly_literal:
            ...
            self.result_code = code.get_py_const(
                py_object_type, "tuple", cleanup_level=2
            )
            code = code.get_cached_constants_writer()
            code.mark_pos(self.pos)
    ...
    def generate_evaluation_code(self, code):
        ...
        for item in self.key_value_pairs:
            ...
            if is_dict:
                ...
                code.putln(
                    "%s.%s = %s;" % (self.result(), item.key.value, item.value.result())
                )
            item.generate_disposal_code(code)
            item.free_temps(code)
...
class PyCFunctionNode(ExprNode, ModuleNameMixin):
    ...
    def analyse_default_args(self, env):
        ...
        if self.def_node.return_type_annotation:
            annotations.append(
                (
                    self.def_node.return_type_annotation.pos,
...
class AIterAwaitExprNode(AwaitExprNode):
    ...
    def yield_from_func(self, code):
        code.globalstate.use_utility_code(
            UtilityCode.load_cached("CoroutineAIterYieldFrom", "Coroutine.c")
        )
        ...
        if self.type.is_complex:
            ...
            if self.operand.type.is_complex:
                real_part = self.type.real_type.cast_code(
                    "__Pyx_CREAL(%s)" % operand_result
                )
                imag_part = self.type.real_type.cast_code(
                    "__Pyx_CIMAG(%s)" % operand_result
                )
            else:
...
class CythonArrayNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln("%s = __pyx_format_from_typeinfo(&%s);" % (format_temp, type_info))
        buildvalue_fmt = " __PYX_BUILD_PY_SSIZE_T " * len(shapes)
        code.putln(
            '%s = Py_BuildValue((char*) "(" %s ")", %s);'
            % (shapes_temp, buildvalue_fmt, ", ".join(shapes))
        )

        err = "!%s || !%s || !PyBytes_AsString(%s)" % (
            format_temp,
            shapes_temp,
            format_temp,
        )
...
class BoolBinopNode(ExprNode):
    ...
    def generate_bool_evaluation_code(
        self, code, final_result_temp, and_label, or_label, end_label, fall_through
    ):
        code.mark_pos(self.pos)
    ...
    def generate_evaluation_code(self, code):
        ...
        self.generate_bool_evaluation_code(
            code, self.result(), and_label, or_label, end_label, end_label
        )
...
class CmpNode(object):
    ...
    def generate_operation_code(self, code, result_code, operand1, op, operand2):
        ...
        if self.special_bool_cmp_function:
            ...
            code.putln(
                "%s = PyObject_RichCompare(%s, %s, %s); %s%s"
                % (
                    result_code,
                    operand1.py_result(),
                    operand2.py_result(),
...
class CoerceToMemViewSliceNode(CoercionNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            "%s = %s(%s);"
            % (self.result(), self.type.from_py_function, self.arg.py_result())
        )
...
class CoerceToBooleanNode(CoercionNode):
    ...
    def generate_result_code(self, code):
        ...
        if test_func is not None:
            code.putln(
                "%s = (%s != Py_None) && (%s(%s) != 0);"
                % (self.result(), self.arg.py_result(), test_func, self.arg.py_result())
            )
        else:
            code.putln(
                "%s = __Pyx_PyObject_IsTrue(%s); %s"
    ...
    def generate_result_code(self, code):
        ...
        if self.use_managed_ref:
            if self.type.is_pyobject:
                ...
                code.put_incref_memoryviewslice(
                    self.result(), not self.in_nogil_context
                )
...
raise_too_many_values_to_unpack = UtilityCode.load_cached(
    "RaiseTooManyValuesToUnpack", "ObjectHandling.c"
)
raise_need_more_values_to_unpack = UtilityCode.load_cached(
    "RaiseNeedMoreValuesToUnpack", "ObjectHandling.c"
)
tuple_unpacking_error_code = UtilityCode.load_cached(
    "UnpackTupleError", "ObjectHandling.c"
)
