...
class MemoryViewSliceType(PyrexType):
    ...
    def same_as_resolved_type(self, other_type):
        return (
            ...
            and self.dtype.same_as(other_type.dtype)
    ...
    def declaration_code(self, entity_code, for_display=0, dll_linkage=None, pyrex=0):
        ...
        return self.base_declaration_code(MemoryView.memviewslice_cname, entity_code)
    ...
    def attributes_known(self):
        if self.scope is None:
            ...
            self.scope = scope = Symtab.CClassScope(
                "mvs_class_" + self.specialization_suffix(), None, visibility="extern"
            )
    ...
    def create_to_py_utility_code(self, env):
        self._dtype_to_py_func, self._dtype_from_py_func = self.dtype_object_conversion_funcs(
            env
        )
        return True
...
class PythranExpr(CType):
    ...
    def attributes_known(self):
        if self.scope is None:
            ...
            # rank 3 == long
            scope.declare_var(
                "shape", CPtrType(CIntType(3)), None, cname="_shape", is_cdef=True
            )
...
class CConstType(BaseType):
    ...
    def declaration_code(self, entity_code, for_display=0, dll_linkage=None, pyrex=0):
        if for_display or pyrex:
            return "const " + self.const_base_type.declaration_code(
                entity_code, for_display, dll_linkage, pyrex
            )
        else:
            return self.const_base_type.declaration_code(
                "const %s" % entity_code, for_display, dll_linkage, pyrex
            )
...
class CFuncType(CType):
    ...
    def same_c_signature_as_resolved_type(
        self, other_type, as_cmethod=0, as_pxd_definition=0
    ):
    ...
    def same_as_resolved_type(self, other_type, as_cmethod=0):
        return (
            self.same_c_signature_as_resolved_type(other_type, as_cmethod)
            and self.nogil == other_type.nogil
        )

    def pointer_assignable_from_resolved_type(self, other_type):
        return self.same_c_signature_as_resolved_type(other_type) and not (
            self.nogil and not other_type.nogil
        )
    ...
    def get_fused_types(self, result=None, seen=None, subtypes=None):
        ...
        return super(CFuncType, self).get_fused_types(result, seen, subtypes=["args"])
    ...
    def same_as_resolved_type(self, other_type):
        if other_type.is_cpp_class:
            if self == other_type:
                ...
                self.cname == other_type.cname
                and self.template_type
...
class CEnumType(CType):
    ...
    def from_py_call_code(
        ...
        rhs = "%s(%s)" % (from_py_function or self.from_py_function, source_code)
        return "%s = %s;%s" % (
            result_code,
    ...
    def create_type_wrapper(self, env):
        ...
        env.use_utility_code(
            CythonUtilityCode.load(
                "EnumType",
                "CpdefEnums.pyx",
                context={"name": self.name, "items": tuple(self.values)},
        ...
        if func_type.templates:
            deductions = reduce(
                ...
                [
                    ...
                    for (pattern, actual) in zip(func_type.args, arg_types)
                ],
                {},
            )
...
def independent_spanning_type(type1, type2):
    ...
    if type1 == type2:
        return type1
    elif (type1 is c_bint_type or type2 is c_bint_type) and (
