...
class MemoryViewSliceBufferEntry(Buffer.BufferEntry):
    ...
    def generate_buffer_slice_code(
        self, code, indices, dst, have_gil, have_slices, directives
    ):
        """
...
def get_is_contig_utility(c_contig, ndim):
    ...
    if c_contig:
        utility = load_memview_c_utility(
            "MemviewSliceIsCContig", C, requires=[is_contig_utility]
        )
    else:
        utility = load_memview_c_utility(
            "MemviewSliceIsFContig", C, requires=[is_contig_utility]
        )
...
def get_copy_new_utility(pos, from_memview, to_memview):
    ...
    for (access, packing) in from_memview.axes:
        if access != "direct":
            return error(pos, "cannot handle 'full' or 'ptr' access at this time.")
...
atomic_utility = load_memview_c_utility(
    "Atomics", context, proto_block="utility_code_proto_before_types"
)

memviewslice_init_code = load_memview_c_utility(
    "MemviewSliceInit",
    context=dict(context, BUF_MAX_NDIMS=Options.buffer_max_dims),
    requires=[memviewslice_declare_code, Buffer.acquire_utility_code, atomic_utility],
)
