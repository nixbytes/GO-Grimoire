...
class UtilityCodeBase(object):
    ...
    def load(cls, util_code_name, from_file=None, **kwargs):
        ...
        if not from_file:
            ...
            files = [filename for filename in listing if filename.startswith(prefix)]
            if not files:
...
class UtilityCode(UtilityCodeBase):
    ...
    def inject_unbound_methods(self, impl, output):
        ...
        impl = re.sub(
            r'CALL_UNBOUND_METHOD\(([a-zA-Z_]+),\s*"([^"]+)"((?:,\s*[^),]+)+)\)',
            externalise,
            impl,
        )
...
class FunctionState(object):
    ...
    def new_loop_labels(self):
        ...
        self.set_loop_labels((self.new_label("continue"), self.new_label("break")))
        return old_labels
...
class GlobalState(object):
    ...
    def generate_object_constant_decls(self):
        ...
        for _, cname, c in consts:
            decls_writer.putln("static %s;" % c.type.declaration_code(cname))
...
class CCodeWriter(object):
    ...
    return_from_error_cleanup_label = funccontext_property(
    ...
    def new_yield_label(self):
        return self.funcstate.new_yield_label()
    ...
    def put_or_include(self, code, name):
        ...
        if include_dir and len(code) > 1024:
            include_file = "%s_%s.h" % (
                name,
                hashlib.md5(code.encode("utf8")).hexdigest(),
            )
    ...
    def put_xdecref(self, cname, type, nanny=True, have_gil=True):
        self._put_decref(
            cname, type, nanny, null_check=True, have_gil=have_gil, clear=False
        )
    ...
    def _put_decref(
        ...
        if clear:
            ...
            self.putln("%s_%sDECREF(%s);" % (prefix, X, self.as_pyobject(cname, type)))
    ...
    def _put_var_decref_clear(self, entry, null_check):
        if entry.type.is_pyobject:
            if entry.in_closure:
                ...
                self.putln(
                    "__Pyx_%sCLEAR(%s);" % (null_check and "X" or "", entry.cname)
                )
            else:
                self.putln(
                    "__Pyx_%sDECREF(%s); %s = 0;"
                    % (
                        null_check and "X" or "",
    ...
    def put_pymethoddef(self, entry, term, allow_skip=True):
        ...
        if method_flags:
            ...
            self.putln(
                ...
                % (entry.name, entry.func_cname, "|".join(method_flags), doc_code, term)
            )
...
class PyxCodeWriter(object):
    ...
    def insertion_point(self):
        return PyxCodeWriter(self.buffer.insertion_point(), self.level, self.context)
