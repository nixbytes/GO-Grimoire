        ...
        if Options.convert_range and node.target.type.is_int:
            if (
                ...
                return self._transform_range_iteration(
                    node, iterator, reversed=reversed
                )
...
class SwitchTransform(Visitor.EnvTransform):
    ...
    def extract_in_string_conditions(self, string_literal):
        if isinstance(string_literal, ExprNodes.UnicodeNode):
            ...
            characters = list(
                set([characters[i : i + 1] for i in range(len(characters))])
            )
            characters.sort()
            return [
                ExprNodes.CharNode(
...
class InlineDefNodeCalls(Visitor.NodeRefCleanupMixin, Visitor.EnvTransform):
    ...
    def visit_SimpleCallNode(self, node):
        ...
        inlined = ExprNodes.InlinedDefNodeCallNode(
            node.pos, function_name=function_name, function=function, args=node.args
        )
...
class OptimizeBuiltinCalls(
    ...
    def _handle_simple_function_tuple(self, node, function, pos_args):
        ...
        if arg.type is Builtin.list_type:
            ...
            return ExprNodes.PythonCapiCallNode(
                node.pos,
                "PySequence_Tuple",
                self.PySequence_Tuple_func_type,
                args=pos_args,
                is_temp=node.is_temp,
            )
    ...
    def _handle_simple_method_object___neq__(
        self, node, function, args, is_unbound_method
    ):
    ...
    def _handle_simple_method_float___neq__(
        self, node, function, args, is_unbound_method
    ):
        return self._optimise_num_binop("Ne", node, function, args, is_unbound_method)
    ...
    def _optimise_num_binop(self, operator, node, function, args, is_unbound_method):
        ...
        utility_code = TempitaUtilityCode.load_cached(
            "PyFloatBinop" if is_float else "PyIntBinop",
            "Optimize.c",
        ...
        return self._substitute_method_call(
            node,
            function,
            "__Pyx_Py%s_%s%s" % ("Float" if is_float else "Int", operator, arg_order),
            self.Pyx_PyFloat_BinopInt_func_type
            if is_float
            else self.Pyx_PyInt_BinopInt_func_type,
    ...
    def _inject_unicode_character_conversion(
        self, node, function, args, is_unbound_method
    ):
        if is_unbound_method or len(args) != 1:
            return node
        ...
        func_call = self._substitute_method_call(
            ...
            function,
            function_name,
            self.PyUnicode_uchar_conversion_func_type,
            method_name,
            is_unbound_method,
            [uchar],
        )
    ...
    PyUnicode_AsXyzString_func_type = PyrexTypes.CFuncType(
        Builtin.bytes_type, [PyrexTypes.CFuncTypeArg("obj", Builtin.unicode_type, None)]
    )
...
class ConstantFolding(Visitor.VisitorTransform, SkipDeclarations):
    ...
    def visit_FormattedValueNode(self, node):
        ...
        if (
            ...
            and node.conversion_char is None
            and isinstance(node.value, ExprNodes.UnicodeNode)
        ):
            return node.value
        return node
    ...
    def visit_JoinedStrNode(self, node):
        ...
        for is_unode_group, substrings in itertools.groupby(
            ...
            if is_unode_group:
                ...
                unode = substrings[0]
                if len(substrings) > 1:
                    unode.value = EncodedString(
                        u"".join(value.value for value in substrings)
                    )
...
class FinalOptimizePhase(Visitor.CythonTransform, Visitor.NodeRefCleanupMixin):
    ...
    def visit_SimpleCallNode(self, node):
        ...
        if function.type.is_cfunction and function.is_name:
            ...
            self.current_directives.get("optimize.unpack_method_calls")
            and node.is_temp
            ...
            if isinstance(node.arg_tuple, ExprNodes.TupleNode) and not (
                ...
                or (node.arg_tuple.is_literal and node.arg_tuple.args)
            ):
