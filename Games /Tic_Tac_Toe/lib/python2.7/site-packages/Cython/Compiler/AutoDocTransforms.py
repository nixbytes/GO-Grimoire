...
class EmbedSignature(CythonTransform):
    ...
    def _fmt_expr_node(self, node, precedence=0):
        if isinstance(node, ExprNodes.BinopNode) and not node.inplace:
            ...
            result = "%s %s %s" % (
                self._fmt_expr_node(node.operand1, new_prec),
                node.operator,
    ...
    def _fmt_arglist(
        self, args, npargs=0, pargs=None, nkargs=0, kargs=None, hide_self=False
    ):
        arglist = []
        for arg in args:
    ...
    def _fmt_signature(
        ...
        arglist = self._fmt_arglist(
            args, npargs, pargs, nkargs, kargs, hide_self=hide_self
        )
