...
class Context(object):
        ...
        if not scope.pxd_file_loaded:
            ...
            if not pxd_pathname:
                ...
                pxd_pathname = self.find_pxd_file(
                    qualified_name, pos, sys_path=need_pxd
                )
                if debug_find_module:
    ...
    def find_pxd_file(self, qualified_name, pos, sys_path=True):
        ...
        pxd = self.search_include_directories(
            qualified_name, ".pxd", pos, sys_path=sys_path
        )
        if pxd is None:  # XXX Keep this until Includes/Deprecated is removed
            if qualified_name.startswith("python") or qualified_name in (
                "stdlib",
                "stdio",
                "stl",
            ):
                standard_include_path = os.path.abspath(
                    os.path.normpath(
                        os.path.join(
                            os.path.dirname(__file__), os.path.pardir, "Includes"
                        )
                    )
                )
                deprecated_include_path = os.path.join(
                    standard_include_path, "Deprecated"
                )
                ...
                if pxd:
                    ...
                    if name.startswith("python"):
                        ...
                        warning(
                            pos, "'%s' is deprecated, use 'libc.%s'" % (name, name), 1
                        )
                    elif name in ("stl"):
    ...
    def search_include_directories(
        self, qualified_name, suffix, pos, include=False, sys_path=False
    ):
        return Utils.search_include_directories(
            tuple(self.include_directories),
            qualified_name,
            suffix,
            pos,
            include,
