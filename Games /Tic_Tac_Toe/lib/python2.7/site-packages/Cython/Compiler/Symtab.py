...
class Scope(object):
    ...
    def declare(
        ...
        if name and name in entries and not shadow:
            ...
            if (
                self.is_cpp_class_scope
                and type.is_cfunction
                and old_type.is_cfunction
                and type != old_type
            ):
    ...
    def lookup_operator(self, operator, operands):
        if operands[0].type.is_cpp_class:
            ...
            if method is not None:
                ...
                res = PyrexTypes.best_match(
                    [arg.type for arg in operands[1:]], method.all_alternatives()
                )
                if res is not None:
                    return res
        ...
        if function is None:
            return None
        return PyrexTypes.best_match(
            [arg.type for arg in operands], function.all_alternatives()
        )
...
class BuiltinScope(Scope):
    ...
    def declare_builtin_type(
        self, name, cname, utility_code=None, objstruct_cname=None
    ):
        name = EncodedString(name)
...
class ModuleScope(Scope):
    ...
    def add_include_file(self, filename):
        if (
            filename not in self.python_include_files
...
class CClassScope(ClassScope):
    ...
    def add_cfunction(self, name, type, pos, cname, visibility, modifiers):
        ...
        entry = ClassScope.add_cfunction(
            self, name, type, pos, cname, visibility, modifiers
        )
        entry.is_cmethod = 1
    ...
    def declare_property(self, name, doc, pos):
        ...
        entry.is_property = 1
        entry.doc = doc
        entry.scope = PropertyScope(
            name, outer_scope=self.global_scope(), parent_scope=self
        )
...
class CppClassScope(Scope):
    ...
    def declare_var(
        ...
        if name != "this" and (defining or name != "<init>"):
            self.var_entries.append(entry)
        if type.is_pyobject and not allow_pyobject:
            error(pos, "C++ class member cannot be a Python object")
        return entry
