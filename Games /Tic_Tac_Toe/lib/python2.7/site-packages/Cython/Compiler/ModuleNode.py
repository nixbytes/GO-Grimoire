...
class ModuleNode(Nodes.Node, Nodes.BlockNode):
    ...
    def merge_in(self, tree, scope, merge_scope=False):
        ...
        extend_if_not_in(self.scope.python_include_files, scope.python_include_files)
        ...
        if api_vars or api_funcs or api_extension_types:
            ...
            if api_vars:
                ...
                for entry in api_vars:
                    ...
                    h_code.putln("#define %s (*%s)" % (entry.name, cname))
            h_code.put(
                UtilityCode.load_as_string("PyIdentifierFromString", "ImportExport.c")[
                    0
                ]
            )
            ...
            for entry in api_extension_types:
                self.generate_type_import_call(
                    entry.type, h_code, "if (!%s) goto bad;" % entry.type.typeptr_cname
                )
    ...
    def generate_c_code(self, env, options, result):
        ...
        code.putln(
            "int %s%s = 0;"
            % (Naming.module_is_main, self.full_module_name.replace(".", "__"))
        )
        code.putln("")
    ...
    def generate_declarations_for_modules(self, env, modules, globalstate):
        ...
        for module in modules:
            ...
            modulecode.putln("")
            modulecode.putln(
                "/* Module declarations from '%s' */" % module.qualified_name
            )
...
"""
    ...
    def generate_objstruct_definition(self, type, code):
        ...
        for attr in type.scope.var_entries:
            if attr.is_declared_generic:
                ...
                attr_type = attr.type
            code.putln("%s;" % attr_type.declaration_code(attr.cname))
        code.putln(footer)
    ...
    def generate_new_function(self, scope, code, cclass_entry):
        ...
        if scope.is_internal:
            ...
            py_attrs = []
        cpp_class_attrs = [
            entry for entry in scope.var_entries if entry.type.is_cpp_class
        ]

        new_func_entry = scope.lookup_here("__new__")
        if base_type or (
            new_func_entry
            and new_func_entry.is_special
    ...
    def generate_dealloc_function(self, scope, code):
        ...
        for entry in py_attrs:
            code.put_xdecref_clear(
                "p->%s" % entry.cname, entry.type, nanny=False, clear_before_decref=True
            )

        for entry in memoryview_slices:
            code.put_xdecref_memoryviewslice("p->%s" % entry.cname, have_gil=True)
        ...
        if base_type:
            ...
            if freelist_size:
                ...
                if is_final_type:
                    ...
                    type_safety_check = " & ((Py_TYPE(o)->tp_flags & (Py_TPFLAGS_IS_ABSTRACT | Py_TPFLAGS_HEAPTYPE)) == 0)"
    ...
    def generate_getattro_function(self, scope, code):
        ...
        def lookup_here_or_base(n, type=None):
            ...
            if r is None and type.base_type is not None:
    ...
    def generate_typeobj_definition(self, modname, entry, code):
        ...
        code.putln('"%s.%s", /*tp_name*/' % (self.full_module_name, scope.class_name))
        if type.typedef_flag:
    ...
    def generate_module_init_func(self, imported_modules, env, code):
        ...
        code.putln(
            'PyErr_SetString(PyExc_ImportError, "init %s");' % env.qualified_name
        )
        code.putln("}")
    ...
    def generate_module_import_setup(self, env, code):
        ...
        code.putln(
            code.error_goto_if_neg(
                ...
                % (fq_module_name, env.module_cname),
                self.pos,
            )
        )
        code.putln("}")
        ...
        if entries:
            ...
            for entry in entries:
                ...
                code.putln(
                    ...
                    % (entry.name, entry.cname, signature, code.error_goto(self.pos))
                )
        ...
        if entries:
            ...
            for entry in entries:
                ...
                code.putln(
                    ...
                    % (temp, entry.name, cname, signature, code.error_goto(self.pos))
                )
    ...
    def generate_type_import_code(self, env, type, pos, code):
        ...
        self.generate_type_import_call(
            type, code, code.error_goto_if_null(type.typeptr_cname, pos)
        )
        if type.vtabptr_cname:
            code.globalstate.use_utility_code(
    ...
    def generate_type_import_call(self, type, code, error_code):
        ...
        code.put("%s = __Pyx_ImportType(%s," % (type.typeptr_cname, module_name))
        ...
        code.putln("%i); %s" % (not type.is_external or type.is_subclassed, error_code))
