...
def _analyse_signature_annotation(annotation, env):
    ...
    if arg_type is not None:
        ...
        warning(
            annotation.pos, "Unknown type declaration found in signature annotation"
        )
...
class CheckAnalysers(type):
    ...
    methods = set(["analyse_types", "analyse_expressions", "analyse_target_types"])
    ...
    def analyse(self, env, nonempty=0, is_self_arg=False):
        ...
        if self.type is None:
            ...
            if (
                ...
                and self.base_type.name is None
            ):
        ...
        for declarator in self.declarators:
            ...
            if isinstance(declarator, CFuncDeclaratorNode):
                name_declarator, type = declarator.analyse(
                    base_type, env, directive_locals=self.directive_locals
                )
            else:
...
class FuncDefNode(StatNode, BlockNode):
    ...
    def generate_function_definitions(self, env, code):
        ...
        if not self.return_type.is_void:
            ...
            code.putln(
                "%s%s;" % (self.return_type.declaration_code(Naming.retval_cname), init)
            )
        ...
        acquire_gil_for_var_decls_only = (
            lenv.nogil
            and lenv.has_with_gil_block
            and (have_object_args or used_buffer_entries)
        )

        acquire_gil_for_refnanny_only = (
            lenv.nogil
            and lenv.has_with_gil_block
            and not acquire_gil_for_var_decls_only
        )
        ...
        for entry in lenv.arg_entries:
            if entry.type.is_pyobject:
                if (
                    acquire_gil or len(entry.cf_assignments) > 1
            ...
            #       incref our arguments
                ...
                is_cdef
                and entry.type.is_memoryviewslice
                and len(entry.cf_assignments) > 1
            ):
                code.put_incref_memoryviewslice(
                    entry.cname, have_gil=code.funcstate.gil_owned
                )
        for entry in lenv.var_entries:
        ...
        if code.error_label in code.labels_used:
            ...
            if (
                ...
                or self.return_type.is_memoryviewslice
            ):
    ...
    def generate_arg_none_check(self, arg, code):
        ...
        code.putln(
            ...
            % (max(200, len(arg.name)), arg.name, code.error_goto(arg.pos))
        )
        code.putln("}")
    ...
    def getbuffer_error_cleanup(self, code):
        info = self.local_scope.arg_entries[1].cname
        code.putln("if (%s != NULL && %s->obj != NULL) {" % (info, info))
        code.put_gotref("%s->obj" % info)
        code.putln("__Pyx_DECREF(%s->obj); %s->obj = NULL;" % (info, info))
        code.putln("}")
    ...
    def declare_cpdef_wrapper(self, env):
        if self.overridable:
            ...
            if self.is_static_method:
                ...
                decorators = [
                    DecoratorNode(
                        self.pos, decorator=NameNode(self.pos, name="staticmethod")
                    )
                ]
...
class DefNode(FuncDefNode):
    ...
    def as_cfunction(
        self, cfunc=None, scope=None, overridable=True, returns=None, modifiers=None
    ):
        if self.star_arg:
    ...
    def analyse_declarations(self, env):
        ...
        if self.return_type is py_object_type and self.return_type_annotation:
            if env.directives["annotation_typing"] and not self.entry.is_special:
                _, return_type = _analyse_signature_annotation(
                    self.return_type_annotation, env
                )
    ...
    def analyse_signature(self, env):
        ...
        elif not env.directives["always_allow_keywords"] and not (
            self.star_arg or self.starstar_arg
        ):
...
class DefNodeWrapper(FuncDefNode):
    ...
    def generate_function_header(self, code, with_pymethdef, proto_only=0):
        ...
        if sig.has_generic_args:
            arg_code_list.append(
                "PyObject *%s, PyObject *%s" % (Naming.args_cname, Naming.kwds_cname)
            )
        arg_code = ", ".join(arg_code_list)
    ...
    def generate_stararg_copy_code(self, code):
        if not self.star_arg:
            ...
            code.putln("if (unlikely(PyTuple_GET_SIZE(%s) > 0)) {" % Naming.args_cname)
            code.put(
                '__Pyx_RaiseArgtupleInvalid("%s", 1, 0, 0, PyTuple_GET_SIZE(%s)); return %s;'
        ...
        if self.self_in_stararg and not self.target.is_staticmethod:
            ...
            code.put(
                "%s = PyTuple_New(PyTuple_GET_SIZE(%s)+1); if (unlikely(!%s)) "
                % (
            ...
            code.putln(
                "for (%s=0; %s < PyTuple_GET_SIZE(%s); %s++) {"
    ...
    def generate_tuple_and_keyword_parsing_code(self, args, success_label, code):
        ...
        code.putln(
            "static PyObject **%s[] = {%s,0};"
            % (
                Naming.pykwdlist_cname,
                ",".join(
                    ["&%s" % code.intern_identifier(arg.name) for arg in all_args]
                ),
            )
        )
            ...
            if func:
                ...
                code.putln(
                    ...
                    % (
                        ...
                        rhs,
                        code.error_goto_if(
                            arg.type.error_condition(arg.entry.cname), arg.pos
                        ),
                    )
                )
                if arg.default:
                    ...
                    if arg.type.is_memoryviewslice:
                        code.put_incref_memoryviewslice(arg.entry.cname, have_gil=True)
                    code.putln("}")
            else:
                error(
                    arg.pos,
    ...
    def generate_argument_values_setup_code(self, args, code):
        ...
        for i, arg in enumerate(args):
            if arg.default and arg.type.is_pyobject:
                ...
                code.putln(
                    "values[%d] = %s;" % (i, arg.type.as_pyobject(default_value))
                )
    ...
    def generate_keyword_unpacking_code(
        ...
        code.putln(
            "const Py_ssize_t pos_args = PyTuple_GET_SIZE(%s);" % Naming.args_cname
        )
        ...
        for i in range(max_positional_args - 1, -1, -1):
            code.put("case %2d: " % (i + 1))
            code.putln(
                "values[%d] = PyTuple_GET_ITEM(%s, %d);" % (i, Naming.args_cname, i)
            )
            code.putln("CYTHON_FALLTHROUGH;")
        ...
        if self.num_required_args or max_positional_args > 0:
            ...
            for i, arg in enumerate(all_args[: last_required_arg + 1]):
                ...
                if arg.default:
                    ...
                    code.putln(
                        "if (likely((values[%d] = PyDict_GetItem(%s, %s)) != 0)) kw_args--;"
    ...
    def generate_optional_kwonly_args_unpacking_code(self, all_args, code):
        ...
        if optional_args:
            ...
            code.putln(
                "PyObject* value = PyDict_GetItem(%s, *%s[index]);"
        ...
        if func:
            ...
            code.putln(
                ...
                % (
                    ...
                    rhs,
                    code.error_goto_if(
                        new_type.error_condition(arg.entry.cname), arg.pos
                    ),
                )
            )
        else:
...
class OverrideCheckNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if self.py_func.is_module_scope:
            ...
            code.putln(
                "else if (unlikely(Py_TYPE(%s)->tp_dictoffset != 0)) {" % self_arg
            )
...
class PyClassDefNode(ClassDefNode):
    ...
    def generate_execution_code(self, code):
        ...
        self.class_cell.generate_injection_code(code, self.class_result.result())
    ...
    def analyse_declarations(self, env):
        ...
        if self.base_class_name:
            if self.base_class_module:
                base_class_scope = env.find_imported_module(
                    self.base_class_module.split("."), self.pos
                )
                if not base_class_scope:
                    error(
                        self.pos,
                        "'%s' is not a cimported module" % self.base_class_module,
                    )
                    return
            else:
            ...
            if self.base_class_name == "object":
                ...
                if (
                    base_class_scope is None
                    or base_class_scope.lookup("object") is None
                ):
            ...
            if base_class_scope:
                ...
                if base_class_entry:
                    if not base_class_entry.is_type:
                        error(
                            self.pos, "'%s' is not a type name" % self.base_class_name
                        )
                    elif not base_class_entry.type.is_extension_type and not (
                        base_class_entry.type.is_builtin_type
                        and base_class_entry.type.objstruct_cname
                    ):
                        error(
                            self.pos,
                            "'%s' is not an extension type" % self.base_class_name,
                        )
                        ...
                        base_class_entry.type.scope
                        and base_class_entry.type.scope.directives
                        and base_class_entry.type.is_final_type
                    ):
                        error(
                            self.pos,
                        ...
                        base_class_entry.type.is_builtin_type
                        and base_class_entry.type.name in ("tuple", "str", "bytes")
                    ):
                        error(
                            self.pos,
        ...
        if self.value:
            ...
            if self.return_type.is_memoryviewslice:
                ...
                code.putln(
                    ...
                    % (Naming.retval_cname, self.value.py_result())
                )
...
class AssertStatNode(StatNode):
    ...
    def analyse_expressions(self, env):
        ...
        if self.value:
            ...
            if value.type is Builtin.tuple_type or not value.type.is_builtin_type:
                ...
                self.value = value.analyse_types(
    ...
    def generate_execution_code(self, code):
        ...
        code.putln("if (unlikely(!%s)) {" % self.cond.result())
        if self.value:
            self.value.generate_evaluation_code(code)
            code.putln(
            ...
            code.putln("PyErr_SetNone(PyExc_AssertionError);")
        code.putln(code.error_goto(self.pos))
        code.putln("}")
...
class ForFromStatNode(LoopNode, StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if (
            self.target.type.is_int
            and not self.target.type.signed
            and self.relation2[0] == ">"
        ):
        ...
        if self.py_loopvar_node:
            ...
            code.putln("%s = %s;" % (self.target.result(), loopvar_name))
        self.body.generate_execution_code(code)
        code.put_label(code.continue_label)
        if self.py_loopvar_node:
...
class TryExceptStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        for exit_label, old_label in [
            (except_error_label, old_error_label),
    ...
    def generate_handling_code(self, code, end_label):
        ...
        if self.pattern:
            ...
            for pattern in self.pattern:
                ...
                exc_tests.append(
                    "__Pyx_PyErr_ExceptionMatches(%s)" % pattern.py_result()
                )
...
class TryFinallyStatNode(StatNode):
    ...
    def put_error_catcher(
        self, code, temps_to_clean_up, exc_vars, exc_lineno_cnames, exc_filename_cname
    ):
        code.globalstate.use_utility_code(restore_exception_utility_code)
        code.globalstate.use_utility_code(get_exception_utility_code)
    ...
    def put_error_uncatcher(
        self, code, exc_vars, exc_lineno_cnames, exc_filename_cname
    ):
        code.globalstate.use_utility_code(restore_exception_utility_code)
        code.globalstate.use_utility_code(reset_exception_utility_code)
                ...
                if entry:
                    ...
                    if kind == "struct" or kind == "union":
                        ...
                        entry = module_scope.declare_c_class(
                            name, pos=pos, module_name=module_name
                        )
                    else:
                        submodule_scope = env.context.find_module(
                            name,
                            relative_to=module_scope,
                            pos=self.pos,
                            absolute_fallback=False,
                        )
                        if submodule_scope.parent_module is module_scope:
                            env.declare_module(
                                as_name or name, submodule_scope, self.pos
                            )
                        else:
                            error(
                                pos,
