...
def p_typecast(s):
    ...
    if not is_memslice and not is_template and not is_const and base_type.name is None:
    ...
    while next_start < size:
        ...
        if match is None:
            error_pos = (
                ...
                pos[1] + end,
                pos[2],
            )  # FIXME: handle newlines in string
        ...
        if c == "\\":
            ...
            if part == "{{":
                ...
                if builder.chars:
                    ...
                    builder = StringEncoding.UnicodeLiteralBuilder()
                next_start, expr_node = p_f_string_expr(
...
expr_terminators = cython.declare(set, set([")", "]", "}", ":", "=", "NEWLINE"]))
                ...
                name_list = ExprNodes.ListNode(
                    ...
                    args=[
                        ExprNodes.IdentifierStringNode(
                            pos, value=s.context.intern_ustring("*")
                        )
                    ],
                )
            else:
...
def p_with_items(s, is_async=False):
    ...
    if not s.in_python_file and s.sy == "IDENT" and s.systring in ("nogil", "gil"):
        ...
        if s.sy == ",":
            ...
            body = p_suite(s)
    return Nodes.WithStatNode(
        pos, manager=manager, target=target, body=body, is_async=is_async
    )
...
def p_c_base_type(s, self_flag=0, nonempty=0, templates=None):
    ...
    if s.sy == "(":
        ...
        return p_c_simple_base_type(
            s, self_flag, nonempty=nonempty, templates=templates
        )
...
def p_c_simple_base_type(s, self_flag, nonempty, templates=None):
    ...
    if s.systring == "const":
        s.next()
        base_type = p_c_base_type(
            s, self_flag=self_flag, nonempty=nonempty, templates=templates
        )
...
base_type_start_words = cython.declare(
    set, basic_c_type_names | sign_and_longness_words | set(special_basic_c_types)
)

struct_enum_union = cython.declare(set, set(["struct", "union", "enum", "packed"]))
...
def p_c_simple_declarator(s, ctx, empty, is_type, cmethod_flag, assignable, nonempty):
    ...
    if s.sy == "*":
        ...
        result = Nodes.CPtrDeclaratorNode(
            pos, base=Nodes.CPtrDeclaratorNode(pos, base=base)
        )
    elif s.sy == "&":
        s.next()
        base = p_c_declarator(
...
def p_cdef_extern_block(s, pos, ctx):
    ...
    return Nodes.CDefExternNode(
        pos, include_file=include_file, body=body, namespace=ctx.namespace
    )
...
def p_compiler_directive_comments(s):
    ...
    while s.sy == "commentline":
        ...
        if m:
            ...
            try:
                result.update(
                    Options.parse_directive_list(directives, ignore_unknown=True)
                )
