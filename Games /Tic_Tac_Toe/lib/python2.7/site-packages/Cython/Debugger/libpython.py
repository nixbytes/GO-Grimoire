...
class PyInstanceObjectPtr(PyObjectPtr):
    ...
    def write_repr(self, out, visited):
        ...
        _write_instance_repr(out, visited, cl_name, pyop_in_dict, self.as_address())
...
class PyLongObjectPtr(PyObjectPtr):
    ...
    def proxyval(self, visited):
        ...
        digits = [ob_digit[i] * (1 << (SHIFT * i)) for i in safe_range(abs(ob_size))]
        result = sum(digits)
        if ob_size < 0:
            result = -result
        return result
...
class PyBytesObjectPtr(PyObjectPtr):
    ...
    def __str__(self):
        ...
        return "".join(
            struct.pack("b", field_ob_sval[i]) for i in safe_range(field_ob_size)
        )
