...
def search_include_directories(
    dirs, qualified_name, suffix, pos, include=False, sys_path=False
):
    ...
    for dir in dirs:
        ...
        if not include:
            ...
            if package_dir is not None:
                ...
                if path_exists(path):
                    return path
                path = os.path.join(dir, package_dir, module_name, package_filename)
                if path_exists(path):
                    return path
    return None
...
def is_package_dir(dir_path):
    for filename in ("__init__.py", "__init__.pyc", "__init__.pyx", "__init__.pxd"):
...
def open_source_file(source_filename, mode="r", encoding=None, error_handling=None):
    if encoding is None:
        # Most of the time the coding is unspecified, so be optimistic that
        # it's UTF-8.
        f = open_source_file(
            source_filename, encoding="UTF-8", mode=mode, error_handling="ignore"
        )
    ...
    stream = io.open(
        source_filename, mode=mode, encoding=encoding, errors=error_handling
    )
    skip_bom(stream)
    return stream
