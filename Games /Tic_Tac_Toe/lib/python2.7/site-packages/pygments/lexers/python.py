...
class PythonLexer(RegexLexer):
    ...
    tokens = {
        ...
        "name": [(r"@[\w.]+", Name.Decorator), ("[a-zA-Z_]\w*", Name)],
        "funcname": [
            include("magicfuncs"),
    ...
    def analyse_text(text):
        return shebang_matches(text, r"pythonw?(2(\.\d)?)?") or "import " in text[:1000]
        ...
        for match in line_re.finditer(text):
            ...
            if line.startswith(u">>> ") or line.startswith(u"... "):
                ...
                if curcode:
                    ...
                    curcode = ""
                    insertions = []
                if line.startswith(u"Traceback (most recent call last):") or re.match(
                    u'  File "[^"]+", line \\d+\\n$', line
                ):
                    tb = 1
                    curtb = line
...
class NumPyLexer(PythonLexer):
    ...
    def analyse_text(text):
        return (
            shebang_matches(text, r"pythonw?(2(\.\d)?)?") or "import " in text[:1000]
