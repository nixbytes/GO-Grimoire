...
class BatchLexer(RegexLexer):
    ...
    _label_compound = r"(?:(?:[^%s%s%s+:^)]|\^[%s]?[^)])*)" % (_nl, _punct, _ws, _nl)
    ...
    _core_token_compound = r'(?:(?:(?:\^[%s]?)?[^"%s%s%s)])+)' % (_nl, _nl, _punct, _ws)
    ...
    def _make_arithmetic_state(
        compound, _nl=_nl, _punct=_punct, _string=_string, _variable=_variable, _ws=_ws
    ):
        op = r"=+\-*/!~"
        state = []
        if compound:
...
class PowerShellLexer(RegexLexer):
    ...
    verbs = (
        ...
        "complete compare clear checkpoint aggregate add"
    ).split()
