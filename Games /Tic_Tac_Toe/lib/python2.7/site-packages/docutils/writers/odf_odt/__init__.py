...
class ODFTranslator(nodes.GenericNodeVisitor):
    ...
    def setup_paper(self, root_el):
        ...
        def walk(el):
            if el.tag == "{%s}page-layout-properties" % SNSD[
                "style"
            ] and not el.attrib.has_key("{%s}page-width" % SNSD["fo"]):
    ...
    def add_header_footer(self, root_el):
        ...
        if self.header_content or self.settings.custom_header:
            ...
            if self.settings.custom_header:
                elcustom = self.create_custom_headfoot(
    ...
    def visit_bullet_list(self, node):
        ...
        if self.in_table_of_contents:
            if self.settings.generate_oowriter_toc:
                ...
                if node.has_key("classes") and "auto-toc" in node.attributes["classes"]:
                    el = SubElement(
    ...
    def generate_figure(self, node, source, destination, current_element):
        ...
        if caption is not None:
            ...
            el2 = SubElement(el1, "style:text-properties", attrib=attrib, nsdict=SNSD)
        ...
        el2 = SubElement(el1, "style:graphic-properties", attrib=attrib, nsdict=SNSD)
        attrib = {
    ...
    def visit_raw(self, node):
        if "format" in node.attributes:
            ...
            if "odt" in formatlist:
                rawstr = node.astext()
                attrstr = " ".join(
                    ['%s="%s"' % (k, v) for k, v in CONTENT_NAMESPACE_ATTRIB.items()]
                )
    ...
    def visit_target(self, node):
        ...
        if not (
            node.has_key("refuri") or node.has_key("refid") or node.has_key("refname")
        ):
            pass
        else:
            pass
