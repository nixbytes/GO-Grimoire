...
CONTACT_FIELDS = (
    ({"email": "author_email", "name": "author"}, "author"),
    ({"email": "maintainer_email", "name": "maintainer"}, "maintainer"),
)
...
def handle_requires(metadata, pkg_info, key):
    ...
    if may_requires:
        ...
        if "extras" not in metadata:
            metadata["extras"] = []
        metadata["extras"].extend(
            [key.extra for key in may_requires.keys() if key.extra]
        )
...
def pkginfo_to_dict(path, distribution=None):
    ...
    try:
        with open(
            os.path.join(os.path.dirname(path), "entry_points.txt"), "r"
        ) as ep_file:
    ...
    if "python.exports" in metadata["extensions"]:
        for (ep_script, wrap_script) in (
            ("console_scripts", "wrap_console"),
            ("gui_scripts", "wrap_gui"),
        ):
            if ep_script in metadata["extensions"]["python.exports"]:
                metadata["extensions"]["python.commands"][wrap_script] = metadata[
                    "extensions"
                ]["python.exports"][ep_script]
